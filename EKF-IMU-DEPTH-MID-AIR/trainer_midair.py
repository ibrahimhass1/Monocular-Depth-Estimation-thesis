...
class Trainer:
    def __init__(self, options):
        ...
        # self.device = "mps" if torch.device("mps") else "cpu"
        # self.device = "cpu"
        ...
        # data
        datasets_dict = {"midair": datasets.midair.MidAirDataset}
        self.dataset = datasets_dict[self.opt.dataset]

        print(os.path.join(os.path.dirname(__file__), "splits", self.opt.split, "{}_files.txt"))
        ...
        print(f"file path is {self.opt.data_path}")
        print(f"use_ekf arg: {self.opt.use_ekf}")
        ...
        full_dataset = self.dataset(
            ...
            k_imu_clip=self.opt.k_imu_clip, is_train=True, img_ext=img_ext, select_file_names = True
        )   
        ...
        self.train_loader = DataLoader(
            ...
            num_workers=self.opt.num_workers, pin_memory=True, drop_last=True, collate_fn= custom_collate)
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.num_epochs):
            print(f"epoch number: {self.epoch}/{self.opt.num_epochs}")
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            original_batch_size = self.opt.batch_size
            current_batch_size = len(inputs[('color',0,0)])
            print(current_batch_size < self.opt.batch_size)
            if current_batch_size < self.opt.batch_size:
                self.opt.batch_size = current_batch_size
                for scale in self.opt.scales:
                    h = self.opt.height // (2 ** scale)
                    w = self.opt.width // (2 ** scale)
            ...
            if current_batch_size < original_batch_size:
                self.opt.batch_size = original_batch_size
                for scale in self.opt.scales:
                    h = self.opt.height // (2 ** scale)
                    w = self.opt.width // (2 ** scale)
    ...
    def process_batch(self, inputs):
        ...
        for key, ipt in inputs.items():
            if key in [("preint_imu", -1, 0), ("preint_imu", 0, 1)]:
                for pkey, pipt in inputs[key].items():
                    inputs[key][pkey] = pipt.to(self.device, dtype=torch.float32)

            else:
                inputs[key] = ipt.to(self.device, dtype=torch.float32)
        ...
        for key, ipt in outputs.items():
            outputs[key] = ipt.to(self.device, dtype=torch.float32)
    ...
    def predict_imu_poses(self, inputs, outputs, use_ekf=False):
        ...
        # pair_feats = {f_i: inputs["color", f_i, 0] for f_i in self.opt.frame_ids}
        for f_i in [-1,1]:
